...
def pdf_to_tab(patch, pages, put_out):
    # pages = ['5']
    # Инициализация словаря для записи информации о распознанных страницах
    rasp = {}
     # Имя файла Excel для сохранения результатов
    exel_nam = f'{put_out}.xlsx'
     # Счетчик распознанных страниц
    raspozn = 0
    ...
    for pag in tqdm(pages):
        ...
        nerasp = 2
        # print(rasp)
        # Попытка чтения таблиц с текущей страницы PDF
        try:
            ...
            if tables.n > 1:
                tab = 2
                par_st = 0
            # Обработка таблиц на текущей странице
            for g in range(0, len(tables)):
                ...
                df1 = tables[g].df
                # Поиск ключевых слов в таблице для определения ее типа
                # print('df1\n',df1)
                    ...
                    if metod == 1:
                        if len(st) > 0:
                            ...
                            if len(df2.index) > 0:
                                ...
                                ws['B2'] =str(nam)
                                wb.save(exel_nam)
                                # Запись информации о распознанной странице в словарь
